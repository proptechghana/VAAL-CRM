...
export function CalendarView({ leads }: { leads: LeadData[] }) {
    const bookedLeads = leads.filter(l => l.Name && l["Date Booked"]).map((l, i) => ({
        id: i,
        name: l.Name,
    ...
    return (
        <div className="bg-white p-4 sm:p-6 rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#F3F3F3]">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-medium text-gray-900 tracking-tight">Scheduled Calls</h2>
            </div>
                            ...
                            <tr key={lead.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="p-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="break-words">{lead.scheduledFor}</span>
                                    </div>
                                </td>
                            </tr>
                        )) : (
            ...
            <div className="sm:hidden space-y-3">
                {bookedLeads.length > 0 ? bookedLeads.map((lead) => (
                    <div key={lead.id} className="bg-[#f8f6f0] rounded-2xl p-4">
                        ...
                        <p className="text-xs text-gray-500 mb-1.5">{lead.property}</p>
